...
func (p *usFsStatCollector) Get() (metrics []*Metric) {
	metricsDir := getFuseMetricDirs()
	fsClientInfo := new(fuseInfo)
	for _, dir := range metricsDir {
		if !utils.IsFileExisting(dir) {
			continue
		}
		...
		for _, subdir := range subdirs {
			if utils.IsFileExisting(filepath.Join(dir, subdir, utils.PodInfoFile)) {
				...
				metrics = append(metrics, p.updateExclusiveMetrics(dir, subdir, fsClientInfo)...)
				continue
			}
			// shared metrics case, subdir is sha256(pvname)
			metrics = append(metrics, p.updateSharedMetrics(dir, subdir, fsClientInfo)...)
		}
	}
	return
}

func (p *usFsStatCollector) updateExclusiveMetrics(fuseMetricsDir, podUid string, fsClientInfo *fuseInfo) (metrics []*Metric) {
	//get pod info
...
func (p *usFsStatCollector) updateSharedMetrics(fuseMetricsDir, subDir string, fsClientInfo *fuseInfo) (metrics []*Metric) {

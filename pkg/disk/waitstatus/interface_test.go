...
func (c *fakeClient) DescribeDisks(req *ecs.DescribeDisksRequest) (*ecs.DescribeDisksResponse, error) {
	var diskIDs []string
	err := json.Unmarshal([]byte(req.DiskIds), &diskIDs)
	if err != nil {
		panic(err)
	}
	resp := &ecs.DescribeDisksResponse{}
...
func isNextStatus(previousStatus string, instanceID string) DiskStatusPredicate {
...
func testEachImpl(t *testing.T, clk clock.WithTicker, f func(*testing.T, *fakeClient, DiskStatusWaiter)) {
...
func TestWaitForDisks(t *testing.T) {
	...
	testEachImpl(t, clk, func(t *testing.T, client *fakeClient, waiter DiskStatusWaiter) {
		...
		defer cancel()
		disk, err := waiter.WaitForDisk(ctx, "d1", isNextStatus("Attaching", "i1"))
...
func TestWaitForDisksCancel(t *testing.T) {
	testEachImpl(t, clock.RealClock{}, func(t *testing.T, client *fakeClient, waiter DiskStatusWaiter) {
		client.disks.Store("d1", disk("d1", "Attaching"))
...
func TestWaitForDisksUnfound(t *testing.T) {
	testEachImpl(t, clock.RealClock{}, func(t *testing.T, client *fakeClient, waiter DiskStatusWaiter) {
		disk, err := waiter.WaitForDisk(context.Background(), "d1", isNextStatus("Attaching", "i1"))
		assert.NoError(t, err)
		assert.Nil(t, disk)
	})
}

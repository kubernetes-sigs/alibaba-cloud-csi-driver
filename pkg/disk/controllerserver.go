...
func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if err := cs.Driver.ValidateControllerServiceRequest(csi.ControllerServiceCapability_RPC_CREATE_DELETE_VOLUME); err != nil {
		klog.Errorf("CreateVolume: driver not support Create volume: %v", err)
		return nil, err
	}
	...
	if req.GetCapacityRange() == nil {
		klog.Errorf("CreateVolume: error Capacity from input: %s", req.Name)
	...
	klog.Infof("CreateVolume: volume: %s created diskpl: %s", req.GetName(), diskPL)
	if diskPL != "" {
	...
	klog.Infof("CreateVolume: Successfully created Disk %s: id[%s], zone[%s], disktype[%s], size[%d], snapshotID[%s]", req.GetName(), diskID, diskVol.ZoneID, diskType, requestGB, snapshotID)
...
func (cs *controllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	// Step 1: check inputs
	if err := cs.Driver.ValidateControllerServiceRequest(csi.ControllerServiceCapability_RPC_CREATE_DELETE_VOLUME); err != nil {
		klog.Warningf("DeleteVolume: invalid delete volume req: %v", req)
...
func (cs *controllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	...
	if params.InstantAccess && disks[0].Category != DiskESSD && disks[0].Category != DiskESSDAuto {
		klog.Warningf("CreateSnapshot: Snapshot(%s) set as not IA type, because disk Category %s", req.Name, disks[0].Category)
...
func snapshotBeforeDelete(volumeID string, ecsClient *ecs.Client) error {
	...
	if disk.Category != DiskESSD && disk.Category != DiskESSDAuto {
		klog.Infof("snapshotBeforeDelete: only supports essd type which current disk.Catagory is: %s", disk.Category)
		return nil
	}
...
func (cs *controllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	...
	case snapNum > 1:
		klog.Errorf("DeleteSnapshot: snapShot cannot be deleted %s, with more than 1 snapshot", snapshotID)
	...
	// log.Log snapshot
	klog.Infof("DeleteSnapshot: Snapshot %s exist with Info: %+v, %+v", snapshotID, existsSnapshots[0], err)
...
func (cs *controllerServer) ListSnapshots(ctx context.Context, req *csi.ListSnapshotsRequest) (*csi.ListSnapshotsResponse, error) {
	...
	case snapNum > 1:
		klog.Errorf("ListSnapshots:: Find Snapshot id[%s], but get more than 1 instance", req.SnapshotId)
		err := status.Error(codes.Internal, "ListSnapshots:: Find Snapshot id but get more than 1 instance")
		utils.CreateEvent(cs.recorder, ref, v1.EventTypeWarning, snapshotTooMany, err.Error())
		return nil, err
	case err != nil:
		klog.Errorf("CreateSnapshot:: Expect to find Snapshot id[%s], but get error: %v", req.SnapshotId, err)
		e := status.Errorf(codes.Internal, "ListSnapshots:: Expect to find Snapshot id but get error: %v", err.Error())
		utils.CreateEvent(cs.recorder, ref, v1.EventTypeWarning, snapshotCreateError, e.Error())
		return nil, e
	}
	volumeID := req.GetSourceVolumeId()
	klog.Infof("ListSnapshots: failed to get snapshot with snapshotid: %s, start get snapshot by volumeid: %s", snapshotID, volumeID)
	if len(volumeID) == 0 {
		snapshotRegion, volumeID, cTime := getSnapshotInfoByID(snapshotID)
		klog.Infof("ListSnapshots:: snapshotRegion: %s, snapshotID: %v", snapshotRegion, snapshotID)
		if snapshotRegion != "" {
			csiSnapshot := &csi.Snapshot{
				SnapshotId:     snapshotID,
				SourceVolumeId: volumeID,
...
func checkInstallDefaultVolumeSnapshotClass(snapClient *snapClientset.Clientset) {
	...
	if err != nil {
		...
		if err != nil {
			klog.Errorf("checkInstallDefaultVolumeSnapshotClass:: failed to create volume snapshot class: %v", err)
		}
	}
}
...
func checkInstallCRD(crdClient *crd.Clientset) {
	...
	if err != nil {
		klog.Errorf("checkInstallCRD:: list CustomResourceDefinitions error: %v", err)
		return
	}
	for _, crd := range crdList.Items {
		if len(crd.Spec.Versions) == 1 && crd.Spec.Versions[0].Name == "v1beta1" {
			klog.Infof("checkInstallCRD:: need to update crd version: %s", crd.Name)
			continue
		}
	...
	kVersion := ""
	if err != nil || info == nil {
		klog.Errorf("checkInstallCRD: get server version error : %v", err)
		kVersion = "v1.18.8-aliyun.1"
	} else {
		kVersion = info.GitVersion
	}
	klog.Infof("checkInstallCRD: need to create crd counts: %v", len(snapshotCRDNames))
	for _, value := range snapshotCRDNames {
		...
		if err != nil {
			klog.Errorf("checkInstallCRD: yaml unmarshal error: %v", err)
			return
		}
		force := true
		...
		if err != nil {
			klog.Infof("checkInstallCRD: crd apply error: %v", err)
			return
		}
	}
}

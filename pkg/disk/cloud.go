...
type attachResult struct {
	devicePath string // only available if called from node
	disk       *ecs.Disk
}

func mapAttachError(err error, nodeID, diskID string) error {
	if aliErr, ok := errors.AsType[*alierrors.ServerError](err); ok {
...
// Returns device path only if called from node (ad.repo != nil)
func (ad *DiskAttachDetach) attachDisk(ctx context.Context, diskID, nodeID string) (attachResult, error) {
	...
	case alreadyAttached:
		...
		if disk.SerialNumber != "" {
			r.devicePath, err = ad.repo.WaitRootBlock(ctx, disk.SerialNumber)
			return r, err
		}
		r.devicePath, err = ad.repo.GetAttached(logger, diskID)
	...
	if ad.repo != nil {
		r.devicePath, err = ad.repo.findDevice(ctx, diskID, disk.SerialNumber, before)
		if err != nil {

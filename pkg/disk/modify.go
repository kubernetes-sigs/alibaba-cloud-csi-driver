...
func (m *ModifyServer) waitForTask(ctx context.Context, logger logr.Logger, taskID string) (*ecs.Task, error) {
	...
	logger.V(2).Info("task finished", "taskID", task, "status", task.TaskStatus, "finishedTime", task.FinishedTime)
	return task, nil
}
...
waitPrevious:
	...
	if params.BurstingEnabled != nil {
		...
		req.DiskId = diskID
		req.BurstingEnabled = requests.NewBoolean(*params.BurstingEnabled)
		...
		logger.V(2).Info("modified disk bursting enabled")
	}

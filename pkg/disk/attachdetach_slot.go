...
// acquire waits until the block expires.
// occupied is the slot already taken by the caller, it is released if we give up waiting.
func (s *blockable) acquire(ctx context.Context, occupied slot) error {
	for {
		until := s.until.Load()
		if until == nil {
			return nil
...
func (s *independentBlockableSlot[TSlot]) Acquire(ctx context.Context) error {
	...
	return s.blockable.acquire(ctx, s.slot)
}
...
func (s serialAD_DetachSlot) Acquire(ctx context.Context) error {
	...
	return s.blockable.acquire(ctx, s)
}

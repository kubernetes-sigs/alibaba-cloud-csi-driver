...
func TestBuildPodSpecServiceAccount(t *testing.T) {
	...
	spec, err := f.buildPodSpec(config, mounterutils.CustomFuseType, &fpm.FusePodContext{
		VolumeId:          "vol-1",
		NodeName:          "node-1",
...
// A missing image is reported against the fuseType that was actually looked up,
// not against the request field, which an empty FuseType leaves blank — and the
// configmap key in the message is the one thing an operator can act on.
func TestBuildPodSpecNoImageNamesResolvedFuseType(t *testing.T) {
	f := NewCustomFuse(utils.Config{}, nil)
	target := "/var/lib/kubelet/plugins/customfuseplugin.csi.alibabacloud.com/vol-1/globalmount"

	_, err := f.PodTemplateSpec(&fpm.FusePodContext{
		VolumeId:          "vol-1",
		NodeName:          "node-1",
		PodTemplateConfig: &fpm.PodTemplateConfig{},
	}, target)

...
func (f *CustomFuse) PodTemplateSpec(c *fpm.FusePodContext, target string) (*corev1.PodTemplateSpec, error) {
	...
	spec, err := f.buildPodSpec(config, fuseType, c, target)
	if err != nil {
		return nil, err
	}
...
func (f *CustomFuse) AddDefaultMountOptions(options []string, mountFlags []string) []string {
	return options
}

func (f *CustomFuse) buildPodSpec(config fpm.FuseContainerConfig, fuseType string, c *fpm.FusePodContext, target string) (spec corev1.PodSpec, _ error) {
	if config.Image == "" {
		return spec, fmt.Errorf("no image configured for fuseType %q; set image in configmap %s/%s under key fuse-%s",
			fuseType, configMapNamespace, configMapName, fuseType)
	}
	...
	socketPath := mounterutils.GetMountProxySocketPath(c.VolumeId, mounterutils.CustomFuseAttachDir)

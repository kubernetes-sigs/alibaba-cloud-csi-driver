...
// The attach directory is chosen per driver rather than per fuseType: a
// NodeUnstageVolume request carries no fuseType, so every fuseType one driver
// mounts has to share a single directory, or the unmount cannot find the path it
// is looking for. The OSS name predates its second fuseType, and both use it.
// A volumeId belongs to one driver and is mounted ONCE.
const (
	OssFuseAttachDir    = "fuse.ossfs"
	CustomFuseAttachDir = "fuse.customfuse"
)

// GetFuseAttachDir returns the attach directory for fuse volumes, given one of
// the attach dir names above.
func GetFuseAttachDir(attachDir string) string {
	return filepath.Join(GetFuseAttachBaseDir(), attachDir)
}

func GetMountProxySocketPath(volumeId, attachDir string) string {
	volSha := sha256.Sum256([]byte(volumeId))
	return filepath.Join(GetFuseAttachDir(attachDir), hex.EncodeToString(volSha[:]), "mounter.sock")
}

func GetAttachPath(volumeId, attachDir string) string {
	volSha := sha256.Sum256([]byte(volumeId))
	return filepath.Join(GetFuseAttachDir(attachDir), hex.EncodeToString(volSha[:]), "globalmount")
}

...
func TestCreateVolumeResponse(t *testing.T) {
	...
	assert.Equal(t, "fs-456+fset-123", result.VolumeId)
	assert.Equal(t, "bmcpfs", result.VolumeContext["csi.alibabacloud.com/volume-type"])
	assert.Equal(t, "fset-123", result.VolumeContext["csi.alibabacloud.com/fileset-id"])

...
type CPFSFileSetManager interface {
	...
	DescribeFileset(ctx context.Context, fsID, fileSetID string) (*nasclient.DescribeFilesetsResponseBodyEntriesEntrie, error)
}
...
func (m *cpfsFileSetManager) DescribeFileset(ctx context.Context, fsID, fileSetID string) (*nasclient.DescribeFilesetsResponseBodyEntriesEntrie, error) {
	request := &nasclient.DescribeFilesetsRequest{
		FileSystemId: tea.String(fsID),
	...
	if response.Body.Entries == nil || len(response.Body.Entries.Entrie) == 0 {
		return nil, fmt.Errorf("fileset %s not found in filesystem %s", fileSetID, fsID)
	}

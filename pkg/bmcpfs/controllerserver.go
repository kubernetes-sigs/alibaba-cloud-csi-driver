...
func (cs *BMCPFSNodeVolume) HasVolumeAttachmentExpand(prefix string) bool {
	for _, v := range cs.volumesAttached {
		if strings.HasPrefix(v, prefix) {
			return true
		}
...
func (cs *controllerServer) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	...
	if nodeStatus.HasPrefixInUse(cpfsID) && nodeStatus.HasVolumeAttachment(cpfsID) {
		klog.Info("volume is in use and attached, skip detach for cpfsID: %s at node: %s", cpfsID, lingjunInstanceID)
		return &csi.ControllerUnpublishVolumeResponse{}, nil
	}
	// 分支3：inuse 存在，attach 不存在，双重检查后卸载
	if nodeStatus.HasPrefixInUse(cpfsID) && !nodeStatus.HasVolumeAttachment(cpfsID) {
		currentStatus, err := cs.checkRemoteNodeStatusByNodeName(req.NodeId) // TODO: checkRemoteNodeStatusByNodeName
		if err == nil && !currentStatus.HasPrefixInUse(prefix) {
			_ = cs.detachDirectly(ctx, req)

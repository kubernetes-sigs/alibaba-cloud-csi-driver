...
func (ns *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if ns.config.EnablePortCheck && opt.Server != "" && opt.MountType != SkipMountType &&
		...
		if err != nil {
			klog.Errorf("NAS: Cannot connect to nas host: %s", opt.Server)
	...
	if opt.MountType == SkipMountType {
		...
		if err != nil {
			klog.Errorf("NAS: Mount volume(%s) path as tmpfs with err: %v", req.VolumeId, err.Error())
...
func (ns *nodeServer) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	if features.FunctionalMutableFeatureGate.Enabled(features.RundCSIProtocol3) {
		if err := directvolume.Remove(targetPath); err != nil {
			klog.Errorf("NodeUnpublishVolume(rund3.0): Remove mount infomation to DirectVolume failed: %v", err)

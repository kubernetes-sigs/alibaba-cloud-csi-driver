...
func TestGetSkipGlobalMount(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "env not set, default false",
			envValue:   "",
			defaultVal: false,
			want:       false,
		},
		{
			name:       "env not set, default true",
			envValue:   "",
			defaultVal: true,
			want:       true,
		},
		{
			name:       "env=true overrides default false",
			envValue:   "true",
			defaultVal: false,
			want:       true,
		},
		{
			name:       "env=false overrides default true",
			envValue:   "false",
			defaultVal: true,
			want:       false,
		},
		{
			name:       "env=true matches default true",
			envValue:   "true",
			defaultVal: true,
			want:       true,
		},
		{
			name:       "env=1 parsed as true",
			envValue:   "1",
			defaultVal: false,
			want:       true,
		},
		{
			name:       "env=0 parsed as false",
			envValue:   "0",
			defaultVal: true,
			want:       false,
		},
		{
			name:       "invalid value falls back to default false",
			envValue:   "yes",
			defaultVal: false,
			want:       false,
		},
		{
			name:       "invalid value falls back to default true",
			envValue:   "invalid",
			defaultVal: true,
			want:       true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.envValue != "" {
				t.Setenv("OSS_SKIP_GLOBAL_MOUNT", tt.envValue)
			}

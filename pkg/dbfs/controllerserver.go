...
func NewControllerServer(d *csicommon.CSIDriver, client *dbfs.Client, region string) csi.ControllerServer {
	...
	if err != nil {
		log.Log.Fatalf("NewControllerServer: Failed to create config: %v", err)
	}
	clientset, err := kubernetes.NewForConfig(config)
	if err != nil {
		log.Log.Fatalf("NewControllerServer: Failed to create client: %v", err)
	}
...
func (cs *controllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if value, ok := pvcProcessSuccess[req.Name]; ok && value != nil {
		log.Log.Infof("CreateVolume: DBFS Volume %s has Created Already: %v", req.Name, value)
	...
	if err != nil {
		log.Log.Errorf("CreateVolume: error dbfs parameters from input: %s, with error: %v", req.Name, err)
	...
	if err != nil {
		log.Log.Errorf("CreateVolume: Create DBFS %s, with error: %v", req.Name, err)
	...
	log.Log.Infof("CreateVolume: DBFS Volume Provision Successful: %s, with PV: %v", req.Name, tmpVol)
	return &csi.CreateVolumeResponse{Volume: tmpVol}, nil
}

// call dbfs api to delete disk
func (cs *controllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	log.Log.Infof("DeleteVolume: Starting to Delete dbfs volume %s", req.GetVolumeId())
	if strings.HasSuffix(req.VolumeId, "-config") {
		log.Log.Infof("DeleteVolume: dbfs config volume not do delete: %s", req.VolumeId)
	...
	if err != nil {
		log.Log.Errorf("DeleteVolume: describe dbfs(%s) with error: %s", req.VolumeId, err.Error())
	...
	if response.DBFSInfo.Status != "unattached" {
		if response.DBFSInfo.Status == "attached" && len(response.DBFSInfo.EcsList) > 0 {
			...
			log.Log.Infof("DeleteVolume: dbfs %s is attahced to %s, detach it first", req.VolumeId, detachDbfsRequest.ECSInstanceId)
		...
		log.Log.Infof("DeleteVolume: dbfs %s not unattahced, cannot be delete in current status %v", req.VolumeId, response)
	...
	if err != nil {
		log.Log.Errorf("DeleteVolume: delete dbfs volume(%s) with error: %s", req.VolumeId, err.Error())
	...
	log.Log.Infof("DeleteVolume: Successful delete DBFS volume %s", req.VolumeId)
	return &csi.DeleteVolumeResponse{}, nil
}
...
func (cs *controllerServer) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (*csi.ControllerPublishVolumeResponse, error) {
	log.Log.Infof("ControllerPublishVolume: Starting Attach dbfs %s to node %s", req.VolumeId, req.NodeId)
	if strings.HasSuffix(req.VolumeId, "-config") {
		log.Log.Infof("ControllerPublishVolume: DBFS config volume not do attach: %s to node %s", req.VolumeId, req.NodeId)
	...
	if err != nil {
		log.Log.Errorf("ControllerPublishVolume: describe dbfs(%s) volume with error %s", req.VolumeId, err.Error())
	...
	if dbfsInfo.DBFSInfo.Status == "attached" {
		for _, ecsItem := range dbfsInfo.DBFSInfo.EcsList {
			if ecsItem.EcsId == req.NodeId {
				log.Log.Infof("ControllerPublishVolume: DBFS volume %s already attach to node %s", req.VolumeId, req.NodeId)
				return &csi.ControllerPublishVolumeResponse{}, nil
			}
		}
	} else if dbfsInfo.DBFSInfo.Status != "unattached" {
		log.Log.Errorf("ControllerPublishVolume: dbfs(%s) volume can not be attached with status (%v)", req.VolumeId, dbfsInfo)
	...
	log.Log.Infof("ControllerPublishVolume: attachDbfs resp:  %s", resp.GetHttpContentString())
	if err != nil {
		if strings.Contains(err.Error(), "InvalidStatus.DBFS") {
			response, _ := describeDbfs(req.VolumeId)
			if response != nil {
				log.Log.Errorf("ControllerPublishVolume: DBFS(%s) Attach error with InvalidStatus status %s", req.VolumeId, response)
			}
		}
		log.Log.Errorf("ControllerPublishVolume: Attach dbfs(%s) to node(%s) with error: %v", req.VolumeId, req.NodeId, err)
	...
	log.Log.Infof("ControllerPublishVolume: Successful attach dbfs %s to node %s", req.VolumeId, req.NodeId)

	return &csi.ControllerPublishVolumeResponse{}, nil
}

func (cs *controllerServer) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	log.Log.Infof("ControllerUnpublishVolume: Detach DBFS Target %s from %s", req.VolumeId, req.NodeId)

	if strings.HasSuffix(req.VolumeId, "-config") {
		log.Log.Infof("ControllerUnpublishVolume: DBFS config volume just skip, %s", req.VolumeId)
		return &csi.ControllerUnpublishVolumeResponse{}, nil
	}

	// not detach dbfs volume if global config set;
	if !GlobalConfigVar.ADControllerEnable {
		log.Log.Infof("ControllerUnpublishVolume: ADController Disable to detach dbfs: %s from node: %s", req.VolumeId, req.NodeId)
		return &csi.ControllerUnpublishVolumeResponse{}, nil
	}

	// detach dbfs volume is disabled
	if GlobalConfigVar.DBFSDetachDisable {
		log.Log.Infof("ControllerUnpublishVolume: DBFS disable to detach dbfs: %s from node: %s", req.VolumeId, req.NodeId)
		return &csi.ControllerUnpublishVolumeResponse{}, nil
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "EntityNotExist.DBFS") {
			log.Log.Infof("ControllerUnpublishVolume: DBFS not exist %s, just skip the detach (%s)", req.VolumeId, err.Error())
			return &csi.ControllerUnpublishVolumeResponse{}, nil
		}
		log.Log.Errorf("ControllerUnpublishVolume: describe dbfs %s with error: %s", req.VolumeId, err.Error())
	...
	if response.DBFSInfo.Status == "attached" || response.DBFSInfo.Status == "unattached" {
		...
		if nodeAttached == false {
			log.Log.Infof("ControllerUnpublishVolume: dbfs %s not attahced to node %s, skip", req.VolumeId, req.NodeId)
			return &csi.ControllerUnpublishVolumeResponse{}, nil
		}
	} else {
		log.Log.Errorf("ControllerUnpublishVolume: dbfs(%s) status not stable: %v", req.VolumeId, response)
	...
	if err != nil {
		if strings.Contains(err.Error(), "EntityNotExist.DBFS") {
			log.Log.Infof("ControllerUnpublishVolume: DBFS not exist %s, just skip(%s)", req.VolumeId, err.Error())
			return &csi.ControllerUnpublishVolumeResponse{}, nil
		}
		if strings.Contains(err.Error(), "NotAttachedStatus.DBFS") {
			log.Log.Infof("ControllerUnpublishVolume: DBFS not attached %s, just skip(%s)", req.VolumeId, err.Error())
			return &csi.ControllerUnpublishVolumeResponse{}, nil
		}
		log.Log.Errorf("ControllerPublishVolume: detach dbfs(%s) from node(%s) with error: %s", req.VolumeId, req.NodeId, err.Error())
		return nil, status.Error(codes.InvalidArgument, "ControllerPublishVolume detach dbfs "+req.VolumeId+" with error: "+err.Error())
	}

	log.Log.Infof("ControllerUnpublishVolume: Successful Detach dbfs %s from Node %s", req.VolumeId, req.NodeId)
	return &csi.ControllerUnpublishVolumeResponse{}, nil
}

func (cs *controllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) {
	log.Log.Infof("CreateSnapshot is called, do nothing now")
	return &csi.CreateSnapshotResponse{}, nil
}

func (cs *controllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	log.Log.Infof("DeleteSnapshot is called, do nothing now")
...
func (cs *controllerServer) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest,
	...
	log.Log.Infof("ControllerExpandVolume: Expand dbfs volume(%s) to size: %dGi", req.VolumeId, newSize)

	if strings.HasSuffix(req.VolumeId, "-config") {
		log.Log.Infof("ControllerExpandVolume: expand dbfs config volume(%s) just skip", req.VolumeId)
	...
	if err != nil {
		log.Log.Errorf("ControllerExpandVolume: describe dbfs %s with error %s", req.VolumeId, err.Error())
	...
	if err != nil {
		log.Log.Infof("ControllerExpandVolume: DBFS volume(%s) resize with error: %s", req.VolumeId, err.Error())
		return nil, status.Error(codes.InvalidArgument, "ControllerPublishVolume resize dbfs "+req.VolumeId+" error: "+err.Error())
	}
	log.Log.Infof("ControllerExpandVolume: DBFS(%s) resize from %dGB to %dGB", req.VolumeId, oldSize, newSize)
	return &csi.ControllerExpandVolumeResponse{CapacityBytes: volSizeBytes, NodeExpansionRequired: false}, nil
}

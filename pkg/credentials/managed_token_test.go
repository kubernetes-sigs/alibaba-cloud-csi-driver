...
func TestManagedTokenCredential(t *testing.T) {
	...
	credential, err := NewManagedTokenCredential(clock.RealClock{}, tokenFile)
	assert.NoError(t, err)
...
func TestTokenNotExist(t *testing.T) {
	_, err := NewManagedTokenCredential(clock.RealClock{}, "/not/exist")
...
func TestTokenAutoReload(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			cred, err := NewManagedTokenCredential(clk, tokenFile)
			assert.NoError(t, err)

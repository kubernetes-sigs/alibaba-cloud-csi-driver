...
# its resources reach the API server only through an explicit opt-in. The second
# variant turns its controller off, which is the only way to render the node half
# on its own.
#
# The last two turn the chart-wide switches off instead. customfuse honours those
# as well as its own, and each of them renders the half the other leaves behind, so
# without them the two conditional blocks in the customfuse templates are only ever
# validated together.
variant_args=("")
...
variant_args+=(
    ...
    "--set csi.customfuse.enabled=true --set csi.customfuse.controller.enabled=false"
    "--set csi.customfuse.enabled=true --set controller.enabled=false"
    "--set csi.customfuse.enabled=true --set plugin.enabled=false"
)

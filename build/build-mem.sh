...
mv plugin.csi.alibabacloud.com ./build/mem/

if [ "$1" == "" ]; then
  cd ./build/mem/
  docker build -t=registry.cn-hangzhou.aliyuncs.com/acs/csi-memplugin:$version-$GIT_SHA ./
  docker push registry.cn-hangzhou.aliyuncs.com/acs/csi-memplugin:$version-$GIT_SHA
fi

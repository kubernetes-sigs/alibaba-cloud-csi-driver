...
function cluster-setup {
    ...
    for zone in $ACK_ZONE; do
        ...
        local id=$(aliyun vpc CreateVSwitch --RegionId $ACK_REGION \
            --ZoneId $zone \
            --VpcId $VPC_ID \
    ...
    while true; do
        local STATE=$(aliyun --region $ACK_REGION cs GET /clusters/${CLUSTER_ID} | jq -r .state)
...
function ssh-forward-setup {
    ...
    echo "getting SLB ID"
    local RESOURCES=$(aliyun cs GET /clusters/${CLUSTER_ID}/resources)
    ...
    while true; do
        local CURRENT_N_NODES=$(aliyun cs GET /clusters/${CLUSTER_ID}/nodes | jq -r '.nodes | length')
    ...
    local nodes=$(aliyun cs GET /clusters/${CLUSTER_ID}/nodes)
    local node_idx=0
    local node
    for node in $(echo $nodes | jq -r '.nodes[].instance_id'); do
        local PRIVATE_IP=$(echo $nodes | jq -r '.nodes['$node_idx'].ip_address[0]')

        echo "creating vServerGroup for node $node_idx $node ($PRIVATE_IP)"
        local VSG_ID=$(aliyun slb CreateVServerGroup --RegionId $ACK_REGION \
...
function cluster-teardown {
    ...
    while true; do
        local STATE=$(aliyun cs GET /clusters/${CLUSTER_ID} | jq -r .state)
        echo "cluster state: $STATE"
